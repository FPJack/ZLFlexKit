...
class FlexKitDemoViewController: UITableViewController {
    ...
    private let demos: [(title: String, subtitle: String, vc: () -> UIViewController)] = [
        ...
        ("㉑ View — 圆角/渐变/描边/阴影",           "radius / cornerRadii / gradColors / border / shadow", { ViewDemoVC() }),
    ]
...
// MARK: - ㉑ View — 圆角 / 渐变 / 描边 / 阴影

class ViewDemoVC: UIViewController {

    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = .compatBackground
        let container = makeScrollableContainerStack(padding: 20)

        // ─── ① 统一圆角 radius ────────────────────────────────────────────
        container.addArrangedSubview(sectionLabel("① radius(_ r) — 四角统一圆角"))
        let row1 = makeRow()
        row1.addArrangedSubview(makeCard { v in
        ...
        let row2 = makeRow()
        row2.addArrangedSubview(makeCard { v in
            v.backgroundColor = .systemGreen
            v.cornerRadii(20, 0, 0, 20)     // 左侧两角圆
        })
        row2.addArrangedSubview(makeCard { v in
            v.backgroundColor = .systemGreen
            v.cornerRadii(0, 20, 20, 0)     // 右侧两角圆
        })
        row2.addArrangedSubview(makeCard { v in
            v.backgroundColor = .systemGreen
            v.cornerRadii(30, 0, 30, 0)     // 对角圆
        })
        container.addArrangedSubview(row2)

        // ─── ③ 渐变色 gradColors ──────────────────────────────────────────
        container.addArrangedSubview(sectionLabel("③ gradColors([...]) — 默认左上→右下对角渐变"))
        let row3 = makeRow()
        row3.addArrangedSubview(makeCard { v in
            v.radius(12)
            v.gradColors([.systemRed, .systemOrange])
        })
        row3.addArrangedSubview(makeCard { v in
            v.radius(12)
            v.gradColors([.systemBlue, .systemPurple])
        })
        row3.addArrangedSubview(makeCard { v in
            v.radius(12)
            v.gradColors([.systemGreen, .systemTeal, .systemBlue])
        })
        container.addArrangedSubview(row3)

        // ─── ④ gradDirection 控制渐变方向 ─────────────────────────────────
        container.addArrangedSubview(sectionLabel("④ gradDirection(start:end:) — 自定义渐变方向"))
        let row4 = makeRow()
        row4.addArrangedSubview(makeCard { v in
            v.radius(12)
            v.gradColors([.systemRed, .systemYellow])
            v.gradDirection(start: CGPoint(x: 0, y: 0.5),
                            end:   CGPoint(x: 1, y: 0.5))  // 水平 →
        })
        row4.addArrangedSubview(makeCard { v in
            v.radius(12)
            v.gradColors([.systemBlue, .systemOrange])
            v.gradDirection(start: CGPoint(x: 0.5, y: 0),
                            end:   CGPoint(x: 0.5, y: 1))  // 垂直 ↓
        })
        row4.addArrangedSubview(makeCard { v in
            v.radius(12)
            v.gradColors([.systemPurple, .systemPink])
            v.gradDirection(start: CGPoint(x: 1, y: 0),
                            end:   CGPoint(x: 0, y: 1))    // 反对角 ↘←
        })
        container.addArrangedSubview(row4)

        
        // ─── ⑤ 描边 border ────────────────────────────────────────────────
        container.addArrangedSubview(sectionLabel("⑤ border(color:w:) / borderColor / borderWidth — 描边"))
        let row5 = makeRow()
        row5.addArrangedSubview(makeCard { v in
            v.backgroundColor = .white
            v.radius(12)
            v.border(color: .systemBlue, w: 1.5)
        })
        row5.addArrangedSubview(makeCard { v in
            v.backgroundColor = .white
            v.radius(12)
            v.border(color: .systemRed, w: 3)
        })
        row5.addArrangedSubview(makeCard { v in
            v.radius(12)
            v.gradColors([.systemTeal, .systemBlue])
            v.border(color: .white, w: 2)   // 渐变 + 白边
        })
        container.addArrangedSubview(row5)

        // ─── ⑥ 阴影 shadow ────────────────────────────────────────────────
        container.addArrangedSubview(sectionLabel("⑥ shadowColor / shadowOffset / shadowRadius / shadowOpacity"))
        let row6 = makeRow()
        row6.addArrangedSubview(makeCard { v in
            v.backgroundColor = .white
            v.radius(12)
            v.shadowColor(color: .systemBlue)           // 默认 opacity=0.2 radius=8
        })
        row6.addArrangedSubview(makeCard { v in
            v.backgroundColor = .white
            v.radius(12)
            v.shadowColor(color: .systemRed)
            v.shadowRadius(radius: 16)
            v.shadowOpacity(opacity: 0.4)
        })
        row6.addArrangedSubview(makeCard { v in
            v.backgroundColor = .white
            v.radius(12)
            v.shadowColor(color: .black)
            v.shadowOffset(w: 4, h: 8)
            v.shadowOpacity(opacity: 0.3)
        })
        container.addArrangedSubview(row6)

        // ─── ⑦ 组合：渐变 + 独立圆角 + 描边 + 阴影 ─────────────────────────
        container.addArrangedSubview(sectionLabel("⑦ 组合效果 — 渐变 + cornerRadii + border + shadow"))
        let combo = View()
        combo.translatesAutoresizingMaskIntoConstraints = false
        combo.heightAnchor.constraint(equalToConstant: 80).isActive = true
        combo
            .gradColors([.systemPurple, .systemPink, .systemRed])
            .gradDirection(start: CGPoint(x: 0, y: 0.5), end: CGPoint(x: 1, y: 0.5))
            .cornerRadii(24, 8, 8, 24)          // 左侧大圆角，右侧小圆角
            .border(color: .white, w: 2)
            .shadowColor(color: .systemPurple)
            .shadowRadius(radius: 12)
            .shadowOpacity(opacity: 0.5)
        
        container.addArrangedSubview(combo)

        // ─── ⑧ 注意事项 ───────────────────────────────────────────────────
        container.addArrangedSubview(sectionLabel("""
        ✅ 注意事项：
        · radius / cornerRadii / border / shadow 调用后会触发 backgroundShapeLayer，
          backgroundColor 之后会改为 shapeLayer.fillColor，不再是 layer.backgroundColor
        · gradColors 必须在 radius/cornerRadii 之后调用才能正确裁剪渐变
        · shadowColor 会自动设置 masksToBounds = false，阴影才可见
        · cornerRadii 参数顺序：topLeading, topTrailing, bottomLeading, bottomTrailing
        · 所有方法均支持链式调用（@discardableResult）
        """))
    }
...
//        sv.translatesAutoresizingMaskIntoConstraints = false
    ...
    // ── 辅助：80×80 View 卡片 ─────────────────────────────────────────────
    private func makeCard(_ configure: (View) -> Void) -> UIView {
        let v = View()
        v.translatesAutoresizingMaskIntoConstraints = false
